...
const CartProvider: React.FC = ({ children }) => {
  ...
  const addToCart = useCallback(
    async product => {
      const productExistsOnCart = products.find(p => p.id === product.id);
  ...
  const increment = useCallback(
    async id => {
      const incrementProductQuantity = products.find(
  ...
  const decrement = useCallback(
    async id => {
      const decrementProductQuantity = products.find(p => p.id === id);

      if (decrementProductQuantity) {
        if (decrementProductQuantity.quantity > 1) {
          setProducts(
            products.map(product =>
              product.id === id
                ? { ...product, quantity: product.quantity - 1 }
